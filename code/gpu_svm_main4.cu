...
// read data
void read_CSV(const string &filename, vector<double> &X, vector<int> &Y, size_t &n_features, const int n)
{
...
{
    int n_limit = 10000;
    if(argc > 1) n_limit = std::stoi(argv[1]);
    ...
    vector<int> y_train;
    size_t n_features;
    read_CSV(train_file, x_train, y_train, n_features, n_limit);
    ...
    vector<int> y_test;
    read_CSV(test_file, x_test, y_test, n_features, 60000);
